...
# @Desc      :   1G file continuous cp,dd,tar,zip,unzip
# ############################################
...
function run_test() {
    ...
    for i in $(seq 1 10); do
        dd if=/dev/urandom of=${testfile1} bs=1M count=1000
        CHECK_RESULT $?
        ls -l ${testfile1} | awk '{print$5}' | grep -w 1048576000
        CHECK_RESULT $?
        dd if=/dev/zero of=${testfile1} bs=1M count=1000
        CHECK_RESULT $?
        ls -l ${testfile1} | awk '{print$5}' | grep -w 1048576000
        CHECK_RESULT $?
    done
