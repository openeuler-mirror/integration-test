...
# #############################################
# @CaseName  :   oe_test_ip_cmd_route
# @Author    :   xuchunlin
# @Contact   :   xcl_job@163.com
# @Date      :   2020.05-09
# @License   :   Mulan PSL v2
# @Desc      :   Command test-ip route
# ############################################
...
function run_test() {
    ...
    CHECK_RESULT $?
    ip -6 route add 2001:224::0/64 via 2001:222::1 mtu lock 1500
    CHECK_RESULT $?
    ip -6 route | grep "2001:224::/64" | grep mtu | grep 1500
    CHECK_RESULT $?
    ip -6 route del 2001:224::0/64
    CHECK_RESULT $?
    ip -6 route | grep "2001:224::0/64"
    CHECK_RESULT $? 0 1
