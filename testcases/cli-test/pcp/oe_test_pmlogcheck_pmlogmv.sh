...
function run_test() {
    ...
    pmlogcheck -l $archive_data | grep 'Log Label'
    CHECK_RESULT $?
    pmlogcheck -n /var/lib/pcp/pmns/root $archive_data | grep 'PMID'
    CHECK_RESULT $?
    pmlogcheck -S @08 -T @18 $archive_data
    CHECK_RESULT $?
    pmlogcheck -v $archive_data | grep 'meta'
    CHECK_RESULT $?
    ...
    CHECK_RESULT $?
    pmlogmv -NV $archive_data /var/log/pcp/pmlogger_new/localhost/ | grep 'link'
    CHECK_RESULT $?
