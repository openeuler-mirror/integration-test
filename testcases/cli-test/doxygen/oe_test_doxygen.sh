...
function run_test() {
    ...
    CHECK_RESULT $?
    test -f HTMLheader && test -f HTMLfooter && test -f HTML-mode
    CHECK_RESULT $?
    ...
    CHECK_RESULT $?
    test -f Latexheader && test -f Latex-mode && test -f config_file
    CHECK_RESULT $?
    ...
    CHECK_RESULT $?
    test -f outputFileName
    CHECK_RESULT $?
    grep "accept" outputFileName
    CHECK_RESULT $?
    pkg_version=$(rpm -qa doxygen | awk -F'-' '{print $2}')
    doxygen -v | grep "${pkg_version}"
    CHECK_RESULT $?
...
function post_test() {
    ...
    rm -rf doxygen-mode doxygen-mode.bak rtf rtf-mode HTMLheader HTMLfooter HTML-mode Latexheader \
    Latex-mode config_file rtf-extensions-file test.xml outputFileName doxysearch.db html latex
    DNF_REMOVE
