...
function run_test() {
    ...
    CHECK_RESULT $?
    test "$(rst2s5 -V | grep -Eo 'Docutils [0-9]*\.[0-9]* '| grep -Eo '[0-9]*\.[0-9]*')" == "$(rpm -qa python3-docutils | awk -F "-" '{print$3}')"
    CHECK_RESULT $?
    rst2s5 -h | grep 'Usage'
    CHECK_RESULT $?
    rst2s5 --no-doc-title testfile.rst test5.html
    CHECK_RESULT $?
    rst2s5 --no-doc-info testfile.rst test6.html
    CHECK_RESULT $?
    rst2s5 --section-subtitles testfile.rst test7.html
    CHECK_RESULT $?
    rst2s5 --no-section-subtitles testfile.rst test8.html
    CHECK_RESULT $?
